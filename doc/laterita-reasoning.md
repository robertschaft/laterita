...
The runtime behaviour therefore only shows up in a file compiled by plain `javac`, where the reason string surfaces as an ordinary stack trace instead of a diagnostic.

That the contract is compile-time is what earns the mechanism its place.
Generic code over a type parameter can't tell at definition time whether a particular instantiation will be valid, and with per-monomorphization checking the *user* of the generic with a non-copyable type sees the error at their call site, not in production.
...
The deeper problem is that the two compilers disagree about what is dead.
`javac`'s reachability analysis (JLS 14.21) has no notion of a call that does not return, so a statement following `broken();` is reachable to `javac` while UNR-01 declares the path dead.
Following a `throw`, `javac` reports the unreachable statement itself, and Laterita's unreachability is `javac`'s plus the UNR-01 rule rather than a competing analysis.

A call also drags Java's inference into a place that has no use for it.
