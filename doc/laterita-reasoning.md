...
C++ unbounded overloading and Rust trait-based overloading are both rejected: the former produces unreadable code (`<<` for stream insertion, expression templates), the latter requires the `Add`/`AddAssign`/`Add<&T>` matrix plus coherence rules — more machinery than a Java-shaped language wants. Bounded sugar as in Kotlin, Python, Swift, and C# avoids both: a fixed set of operators desugaring to methods via ordinary overload resolution. None of those languages collapsed into C++ chaos from it.

`@Operator(op)` names the operator and leaves the method name free because Java's ecosystem uses inconsistent arithmetic spellings — `BigDecimal.add`, `Instant.plus`, `Duration.negated` — and binding `+` to a fixed name would exclude the standard-library types operator sugar should serve first. The annotation is required (not name-based) because `add`/`plus` carry multiple meanings (`Collection.add` mutates, `LocalDate.plusDays` is a units helper). An `Arithmetic<Self>` interface is rejected for the same reason `Comparable` works for comparison and nothing works for arithmetic: ordering is one homogeneous operation, but `Money / Money` yields a scalar, `Instant - Instant` yields a `Duration`, and Java cannot implement a generic interface twice with different type arguments. Per-method `@Operator(op)` carries its own signature and has no such limit. Comparison uses `Comparable` with no annotation because `compareTo` has exactly one ecosystem meaning.

Resolution is left-operand-directed with a built-in numeric fallback and no implicit conversion, so `value + literal` and `literal + value` either both succeed (operands reduce to primitives via the delegate) or both fail (non-numeric type, type error on both orderings). A reflected form would buy symmetry at the cost of implicit conversions Laterita avoids everywhere else.
