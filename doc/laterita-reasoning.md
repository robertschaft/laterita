...
### Why `String` exposes no public splitting API (dissolving OQ-17)

OQ-17 asked which shape — multi-return, record, or `unsafe`-internal-only — to use for public buffer splitting on `String`. The question dissolves once STR-07 is taken seriously. `String` declares no `mut` methods at all, so every `bound String` is a read-only view, and multiple read-only borrows of the same source coexist under MOVE-04 without disjointness obligations. The N non-overlapping slices that `String.split`, `Pattern.split`, `String.lines`, `URI` component getters, and `StringTokenizer.nextToken` produce are non-overlapping as a fact about each algorithm, not a fact the type system has to verify. Repeated `substring` calls (STR-03) suffice to implement every case; no `splitAt` machinery needs to surface on `String` at all, and the standard implementation of `String.split` is a single pass that writes successive `substring(prevEnd, matchStart)` views into the result array with no `unsafe` block anywhere.

That collapses the three sub-options. Option 1 (a multi-return language feature for `splitAt`) and option 2 (a `StringSplit` record) are both unmotivated, because `String` has no two-mut-slices-simultaneously case to drive them. Option 3 (keep splitting internal behind existing methods) falls out for free without even the `unsafe` it originally postulated, since `substring` is the only primitive needed. The reason `splitAt` exists at all is the **mut array** case — two simultaneous `mut T[]` slices over the same backing buffer, for parallel in-place algorithms — and that case is about arrays, not strings. Whether and how to expose it lives in OQ-19.
