...
### Why newtypes are field-less subclasses, and why no operator overloading (EXT-01)
...
Second, does the wrapper inherit behavior, and how does math read? It inherits the parent's full method surface by ordinary subtyping. There are no generated delegate methods — delegation is what you write when the wrapper is *not* a subtype, and the whole point of extending the value class is that it *is* one. This is where the operator-overloading question (the other half of this resolution) is settled by omission rather than by adding machinery. C++ made every operator overloadable and the result is `<<` meaning stream insertion and expression templates that no one can read; Rust inherited the trait-based version and with it the orphan rules and the `Add`/`AddAssign`/`Add<&T>` matrix; Java declined the feature entirely and lost only value-typed arithmetic ergonomics. Laterita sides with Java: the operators are Java's fixed set, and a numeric newtype widens to its parent to use them, so `a + b` on two `Meters` has type `Double` and assigning it back to `Meters` is an explicit constructor call. That widening does mean `meters + seconds` type-checks at `Double` — Laterita polices unit-correctness at declaration boundaries (the argument-order bugs that motivate STR-01), not inside arithmetic expressions, because doing the latter is exactly the operator-overloading surface being declined. A domain type that needs arithmetic to stay closed over itself defines named methods on a value class (`Money.add`), the same Java-shaped call every other value class uses. The gain is that one mechanism — field-less value-class subclassing — answers both "give me a distinct zero-cost wrapper" and "give me value-typed math" without a second annotation and without an overloading grammar.
