...
The five `.lat` forms are confined to §19, and §19 carries a hard constraint: every form in it is sugar with an exact desugaring to the §1–18 surface, and none introduces semantics of its own (LAT-00). This keeps the two surfaces from drifting into two languages. A `.lat` source and its desugared `.java` form are the same program; migration tooling rewrites one into the other mechanically in either direction.
...
### Why ownership annotations are not part of overload identity (MOVE-03)
...
### Why newtypes are field-less subclasses, and why operators are bounded sugar (EXT-01, LAT-07)

The microtype pattern STR-01 enables for `String` generalizes: the boxed numerics are value classes (MUT-03), and the same argument that makes `class Email extends String` worth its `.lat`-only cost makes `class UserId extends Long` and `class Meters extends Double` worth theirs. Rust reaches for `struct UserId(u64)` precisely because a type alias is not a type; Laterita gets the same distinct-type guarantee from a subclass.

Two questions then decide the shape. First, does the wrapper cost anything? It need not. A subclass that adds no field has the same representation as its parent, and AOT compilation (COMP-01) erases it — there is no object header to pay for and no JVM value-class machinery to wait on, which is why Laterita does not need Valhalla to make the pattern zero-cost. The field-less condition is exactly the condition under which erasure is sound, so it is also the trigger: a subclass with a field has its own layout and is an ordinary value subclass. Because "did you declare a field" is already visible in the source, no `@newtype` annotation is needed to mark the guarantee — an annotation would only restate what the field list already says, and the design avoids annotations that carry no information the compiler cannot otherwise see.

Second, does the wrapper inherit behaviour? It inherits the parent's full method surface by ordinary subtyping. There are no generated delegate methods — delegation is what you write when the wrapper is *not* a subtype, and the whole point of extending the value class is that it *is* one. So `class Money extends BigDecimal` already reads `a.add(b)`; the open question is only whether `a + b` may stand for it.
...
LAT-07 takes the bounded shape, and the gating choices follow from one observation: the method *name* is the wrong place to encode the operator. Binding `+` to a fixed name — Kotlin's `plus`, or even Java's `add` — excludes the many Java types whose arithmetic is spelled otherwise: the `java.time` types use `plus`/`minus`, `Duration` negates with `negated()`, and a fixed-name rule would force wrappers or renames on exactly the standard-library types operator sugar should serve first. So `@Operator(op)` names the operator and leaves the method name free, making `BigDecimal.add`, `Instant.plus`, and `Duration.negated` eligible under their existing signatures — and the desugaring is still exactly the call a `.java` mirror writes. The annotation is *required* (rather than blessing names) precisely because names like `add`/`plus` carry several meanings across the ecosystem — `Collection.add` mutates, `LocalDate.plusDays` is a units helper — so eligibility-by-name would attach `+` to methods that are not arithmetic; opt-in removes that hazard. The operator method is an instance method on the operand's own type, never `static`: a static or free-standing operator could be declared anywhere, so `a + b`'s meaning would not be findable from `a`'s type — requiring an instance method keeps the definition where the reader looks for it.

Comparison is gated differently, and deliberately. `< <= > >=` desugar through `Comparable.compareTo` with no annotation, because `compareTo` — unlike `add`/`plus` — has exactly one meaning in the Java ecosystem (total ordering). Implementing `Comparable` is therefore already an unambiguous opt-in, and a second marker would be redundant; `==`/`!=` are left on their Java meaning since reference-vs-value equality is a separate question (NULL/OBJ surface), not an ordering. The operator set stops at `+ - * /`, unary `-`, and the four relationals: `%` is integer-only and rarely wanted on value types, `[]` would advertise the constant-time indexing a general method cannot promise, and compound assignment is left out to keep the surface minimal until a concrete need argues otherwise.
