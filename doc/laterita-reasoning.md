...
### Why `synchronized` lowers to `Monitor`, not to `Mutex<T>` (STD-10, THR-11–THR-13)

The Java `synchronized` keyword and `Object.wait` / `notify` / `notifyAll` are preserved and compiled to operations on a stdlib `Monitor` and a paired `Condition` (THR-12, THR-13). The intrinsic per-Object monitor becomes a compiler-managed slot (THR-11). Three earlier objections to keeping `synchronized` dissolve when the lowering target is a data-less reentrant `Monitor` rather than `Mutex<T>`.

**The intrinsic monitor isn't free, so it isn't universal.** Java's `synchronized` works because every `Object` carries a hidden header word the JVM materializes into a monitor on first contention. In an AOT-compiled language with no GC (COMP-01), giving every allocation an intrinsic-lock slot is a per-object cost paid by code that never locks anything. THR-11 sidesteps this two ways: the slot is lazily initialized (no allocation cost until first acquisition), and implementations are explicitly permitted to omit it for classes the whole-program analysis proves are never used as synchronization targets. The remaining per-object header pointer is the price of preserving the Java source surface without an asterisk.

**Reentrancy doesn't collide with borrow exclusivity once the lock is data-less.** Java monitors are reentrant; `Mutex<T>` is not, because re-entering `with` would mean handing out a second mutable borrow of the protected `T` while the first is still live (MOVE-04). `Monitor` (STD-10) carries no protected value and hands out no borrow, so re-entry is sound: nested `with` calls share the lock and the inner body runs without contention. The borrow checker is unaffected — the synchronized block is ordinary code, and mutation inside it still requires the usual `@mut` / `@mutating` access (THR-12).

**`wait` / `notify` get a home.** `Condition` (STD-11) sits next to `Monitor` exactly the way `java.util.concurrent.locks.Condition` sits next to `ReentrantLock`. The anonymous `Condition` attached to each per-object `Monitor` is the lowering target for `Object.wait` / `notify` / `notifyAll`; programs needing multiple conditions per lock construct an explicit `Monitor` and call `newCondition()`. Blocking-interruption semantics (THR-04) are inherited from `Condition.await`; the `onDrop` no-blocking rule (THR-05) continues to apply to user code, so `wait` calls in `onDrop` bodies remain rejected exactly as before.

**`Mutex<T>` stays the preferred primitive for new code.** It binds the lock to a protected value and lets the compiler prove that every access to the protected state happens under the lock — which `synchronized` / `Monitor` cannot. New laterita code uses `Mutex<T>.with`; `Monitor` is the lowering target for Java's intrinsic-monitor semantics and the escape hatch for cases where the lock guards state spread across several fields of `this`, which `Mutex<T>` cannot easily express. The two primitives sit at different points on the same axis — data-bound vs. data-less, non-reentrant vs. reentrant — and the spec carries both for distinct reasons.

The migration story therefore splits cleanly: existing Java code using `synchronized` and `wait` / `notify` compiles unchanged; new laterita code preferring `Mutex<T>` is encouraged but never required.
