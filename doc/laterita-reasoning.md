...
A class with `@local` fields must explicitly choose `@local` (inherit thread-affinity) or `@nonlocal` (encapsulate). The compiler does not infer the choice — making it explicit forces the author to name what they're claiming, and prevents accidentally promoting an `@local`-bearing class into the thread-safe pool by composition. The two annotations on the class are independent of `@unsafe`, which lives only on methods (UNS-01). The author's per-method `@unsafe` carries the compiler-unverifiable operations (UNS-02), and the class-level `@nonlocal` carries the encapsulation claim — one job per annotation.
