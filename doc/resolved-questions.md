...
| Receiver mutation declared by `@mut` on the return type | Rejected. A functional-interface return type carries its slot mode (CLO-03) immediately before the return type; a return-type `@mut` could not distinguish "this method mutates its receiver" from "this method returns a mutating closure," and a mutating closure could not be returned at all. Receiver mutation rides the explicit `this` slot instead, parallel to `@take` (BIND-07). | BIND-05; BIND-07; reasoning "Why methods declare mutation in the signature" |
...
* **OQ-07** — Method-level `mut` syntax. Resolved by BIND-02 and BIND-05: receiver mutation is marked with `@mut` on the method's explicit `this` parameter, parallel to receiver consumption (BIND-07). Laterita introduces no new keyword for it (§18).
