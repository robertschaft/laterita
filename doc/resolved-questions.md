...
| Receiver mutation declared by reusing `@mut` (on the return type or the explicit `this`) | Rejected. A functional-interface return type carries its slot mode (CLO-03) as `@mut` immediately before the return type, so a return-type `@mut` could not distinguish "mutates its receiver" from "returns a mutating closure," and a mutating closure could not be returned at all. Placing it on an explicit `this` avoids that clash but forces every mutating method to spell out a `(@mut Self this)` parameter. Receiver mutation is the dedicated `@mutating` method annotation instead (BIND-05). | BIND-05; reasoning "Why methods declare mutation in the signature" |
