...
Placed on the sole component of a `record`, it causes the compiler to generate, for every `public` instance method of the component's declared type, a forwarding method on the record. Generated methods return the component's type (they *decay*). Ownership annotations (`@consuming`, `@mutating`) are propagated from the source method to the generated forwarder. A method explicitly declared on the record shadows the generated delegate for the same signature. `@Delegate` is rejected if the record has more than one component or zero components. See `GEN-01`.
