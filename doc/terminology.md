...
### newtype
A `record` with exactly one component annotated `@Delegate` — the Laterita form of Rust's `struct Meters(f64)` / `struct UserId(u64)`. The compiler erases the wrapper at the ABI level (`COMP-01`): the record has the same size and alignment as its component. It is a distinct nominal type — not a subtype of the component's type, no implicit widening. `@Delegate` generates forwarding methods from the component's type; they return the component type (decay). A user-declared method shadows a generated delegate for the same signature. `record Email(@Delegate @take String raw)` and `record UserId(@Delegate @take Long raw)` are newtypes. See `NT-01`.
...
### @Delegate (annotation on record components)
Placed on the sole component of a `record`, it makes that record a newtype. The compiler generates, for every `public` instance method of the component's declared type, a forwarding method on the record that invokes the same method on the component. Generated methods return the component's type (they decay). A method explicitly declared on the record shadows the generated delegate for that signature. `@Delegate` is rejected if the record has more than one component or zero components. See `NT-01`.
